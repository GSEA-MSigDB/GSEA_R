...
GSEA.CollapseDataset <- function(dataplatform, gct, collapse.mode) {
 ...
 mappedgct[, c(2:ncol(mappedgct))] <- sapply(mappedgct[, c(2:ncol(mappedgct))], 
  as.numeric)
 mappedgct <- mappedgct[rowSums(is.na(mappedgct[, 2:ncol(mappedgct)])) != ncol(mappedgct[, 
  2:ncol(mappedgct)]), ]

 if (collapse.mode == "max") {
  mappedexp <- mappedgct %>%
   group_by(.data$Gene.Symbol) %>%
   summarise_all(max, na.rm = TRUE) %>%
   data.frame()
 }
 if (collapse.mode == "median") {
  mappedexp <- mappedgct %>%
   group_by(.data$Gene.Symbol) %>%
   summarise_all(median, na.rm = TRUE) %>%
   data.frame()
 }
 if (collapse.mode == "mean") {
  mappedexp <- mappedgct %>%
   group_by(.data$Gene.Symbol) %>%
   summarise_all(mean, na.rm = TRUE) %>%
   data.frame()
 }
 if (collapse.mode == "sum") {
  mappedexp <- mappedgct %>%
   group_by(.data$Gene.Symbol) %>%
